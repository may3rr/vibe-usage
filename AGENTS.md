...
- The current store's positive usage rows satisfy `tokens.total = input + output + cache.read + cache.write + reasoning`: these fields are non-overlapping. Preserve input/output/cache-read/reasoning as separate bucket counters; fold an untyped future `cache.write` into input because the shared schema cannot price an unknown cache-write TTL. Accept `modelID` / `modelId` and nested `model.modelID` / `model.modelId` spellings without renaming concrete provider model ids.
